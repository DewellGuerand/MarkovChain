...
#let slides(
  ...
  theme: "normal",
  logo: none,            // <- NOUVEAU
  logo-width: 1.6cm,      // <- optionnel (ex: 1.6cm)
  logo-height: 0.8*1cm,     // <- optionnel (ex: 0.8*space)
  logo-pos: "right", 
) = {
  ...
  // HEADER
    ...
    #if logo != none {
        // Calcule une taille cohérente si non fournie
        let _lw = if logo-width != none { logo-width } else { none }
        let _lh = if logo-height != none { logo-height } else { 0.8 * space }

        // Choix position
        place(top + right, dx: 0.7cm, dy: 0pt)[
        // Petit logo en coin si souhaité
        #image(logo, height: 0.9*space)
      ]

        // Pour éviter la collision verticale avec le contenu du header,
        // on peut surélever légèrement selon le thème
